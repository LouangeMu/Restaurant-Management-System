<?php 
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(isset($_POST['submit']) && isset($_FILES['file'])) {
            ...
            $restau = htmlentities($_POST['restau'], ENT_QUOTES, 'UTF-8');
			$file = $_FILES['file'];
			$allowed_ext = array("jpg", "jpeg", "JPG", "JPEG", "png", "PNG");
			
			if($cat != "" && $name != "" && $price != "" && $desc != "" && $restau != "" && empty($file) == false) {
				...
				if(in_array($ext[1], $allowed_ext)) {
                    ...
                    {
                        $msg = "<p style='color:red; padding: 10px; background: #ffeeee;'>No digits allowed for name. Try again!!!</p>";

                    } else if(!preg_match('/^[0-9]*$/', $price))
                    {
                        $msg = "<p style='color:red; padding: 10px; background: #ffeeee;'>only digits allowed for price. Try again!!!</p>";
                    }
                    else if(strlen($price)>4)
                    {
                        $msg = "<p style='color:red; padding: 10px; background: #ffeeee;'>please be sure on your price. Try again!!!</p>";
                    } else if(!preg_match("/^[a-zA-Z\s]+$/", $desc))
                    {
                        $msg = "<p style='color:red; padding: 10px; background: #ffeeee;'>No digits allowed for Description. Try again!!!</p>";
                    }
                    else if(!preg_match("/^[a-zA-Z\s]+$/", $restau))
                    {
                        $msg = "<p style='color:red; padding: 10px; background: #ffeeee;'>No digits allowed for restaurant name. Try again!!!</p>";
                    }
                    
                    else{
						
						$insert = $db->query("INSERT INTO food(food_name, food_category, food_price, food_description, food_restaurant) VALUES('".$name."', '".$cat."', '".$price."', '".$desc."', '".$restau."')");
                            ...
                            <div class="content">
                                <form method="post" action="food_add.php" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label style="color: #333" for="txtarea">Restaurant name</label>
                                                <input type="text" autofocus name="restau" class="form-control" placeholder="Enter Restaurant Name" required />
                                            </div>
                                        </div>
                                    </div>
        ...
        <footer class="footer">
            <div class="container-fluid">
                ...
                <p class="copyright pull-right">
                    &copy; <?php echo strftime("%Y", time())?> <a href="index.php">MyRestaurant</a>
                </p>
            </div>
        </footer>
